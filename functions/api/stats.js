...
export async function onRequest(context) {
    ...
    if (request.method === 'POST') {
        ...
        if (existing) {
            const oldJ = JSON.parse(existing.janken);
            const oldA = JSON.parse(existing.acchi);
            mergedJanken = mergeStats(oldJ, janken);
            mergedAcchi = mergeStats(oldA, acchi);
        ...
        return new Response(JSON.stringify({ success: true, user_id, total_plays: totalPlays }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        });
    }
