export async function onRequestGet(context) {
    ...
    const stmt = env.DB.prepare('SELECT janken_stats, acchi_stats FROM user_stats WHERE user_id = ?');
    const result = await stmt.bind(userId).first();
    if (!result) {
        return new Response(JSON.stringify({ janken_stats: null, acchi_stats: null }), { status: 200 });
    }
